  ...
  <div class="page-container">
    ...
    <el-card class="table-card">
      <div class="operation-bar">
        <el-button type="primary" :icon="Plus" @click="handleAdd">新增跟进记录</el-button>
      </div>
    ...
    <el-dialog
      ...
      :title="dialogType === 'add' ? '新增跟进' : '编辑跟进'"
      width="600px"
      @close="resetForm"
    >
      <el-form ref="formRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="带看房源" prop="houseId">
          ...
          >
            <el-option
              ...
              :key="item.id"
              :label="item.houseNo + ' (' + item.layout + ')'"
              :value="item.id!"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="客户反馈" prop="customerFeedback">
          <el-input 
            ...
            type="textarea" 
            :rows="3" 
            placeholder="客户对房源的评价，如价格、户型等" 
          />
        </el-form-item>
        <el-form-item label="跟进建议" prop="followAdvice">
          <el-input 
            v-model="formData.followAdvice" 
            type="textarea" 
            :rows="2" 
            placeholder="后续跟进计划" 
          />
...
import { reqViewRecordPage, reqSaveViewRecord, reqUpdateViewRecord, reqDeleteViewRecord } from '@/api/viewRecord';
...
const formData = reactive<ViewRecord>({
  ...
  salesId: userStore.userInfo.id, // 默认为当前登录用户
  viewTime: '',
  customerFeedback: '',
  followAdvice: ''
});
...
const loadData = async () => {
  loading.value = true;
  try {
    const res = await reqViewRecordPage(queryParams);
    if (res && res.data) {
...
const searchCustomers = async (query: string) => {
  if (query) {
    ...
    try {
      // 复用客户分页接口进行搜索
      const res = await reqCustomerPage({ pageNum: 1, pageSize: 20, realName: query });
      if (res && res.data) {
...
const searchHouses = async (query: string) => {
  if (query) {
    ...
    try {
      ...
      const res = await reqHousePage({ pageNum: 1, pageSize: 20, houseNo: query } as any);
      if (res && res.data) {
...
const handleAdd = () => {
  ...
  resetFormState();
  formData.salesId = userStore.userInfo.id; // 确保销售ID是当前用户
  formData.viewTime = new Date().toLocaleString().replace(/\//g, "-"); // 默认当前时间
...
const submitForm = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const api = dialogType.value === 'add' ? reqSaveViewRecord : reqUpdateViewRecord;
...
const handleDelete = (row: ViewRecord) => {
  ElMessageBox.confirm('确定要删除这条跟进记录吗？', '提示', { type: 'warning' })
    .then(async () => {
      try {
        const res = await reqDeleteViewRecord(row.id!);
        if (res && res.status) {
...
.page-container {
  ...
  .table-card {
    .operation-bar { margin-bottom: 15px; }
