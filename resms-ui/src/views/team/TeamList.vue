<template>
  <div class="team-list-container">
    <el-card class="search-card" shadow="never">
      <el-form :model="queryParams" :inline="true" label-width="80px">
        <el-form-item label="团队名称">
          <el-input v-model="queryParams.teamName" placeholder="请输入团队名称" clearable style="width: 200px"
    ...
    <el-card class="toolbar-card" shadow="never">
      <el-row :gutter="10">
        <el-col :span="1.5">
          <el-button v-if="isAdminFlag" type="primary" :icon="Plus" @click="handleAdd">新增团队</el-button>
        </el-col>
      </el-row>
    </el-card>

    <el-card class="table-card" shadow="never">
      <el-table v-loading="loading" :data="teamList" border stripe>
        ...
        <el-table-column label="目标业绩" prop="performanceTarget" width="140" align="center">
          <template #default="{ row }">
            <span>{{ formatMoney(row.performanceTarget) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" fixed="right" align="center">
          <template #default="{ row }">
            <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
              ...
              <el-tooltip v-if="isAdminFlag" content="删除" placement="top">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="650px" :close-on-click-modal="false"
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="团队名称" prop="teamName">
          <el-input v-model="form.teamName" placeholder="请输入团队名称" :disabled="isManagerFlag" />
        </el-form-item>

        <el-form-item label="目标业绩" prop="performanceTarget">
          <el-input-number v-model="form.performanceTarget" :min="0" :precision="2" :controls="false" style="width: 100%">
            <template #prefix>￥</template>
          </el-input-number>
        </el-form-item>

        <el-form-item label="团队经理" prop="managerId">
          <el-select v-model="form.managerId" placeholder="请选择销售经理" clearable filterable style="width: 100%" :disabled="isManagerFlag">
            <el-option v-for="item in managerOptions" :key="item.id" :label="item.realName" :value="item.id" />
          </el-select>
        </el-form-item>

        <el-form-item label="团队成员" prop="memberIds">
          <el-transfer v-model="form.memberIds" filterable filter-placeholder="搜索成员" :titles="['待选销售', '已选成员']"
            :button-texts="['移除', '添加']" :data="salesOptions" :props="{ key: 'id', label: 'realName' }" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="detailVisible" title="团队详情" width="500px">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="目标业绩">{{ formatMoney(currentDetail?.performanceTarget) }}</el-descriptions-item>
        <el-descriptions-item label="成员列表">
          <div v-if="currentDetail?.members && currentDetail.members.length > 0" class="member-tags">
            <el-tag v-for="member in currentDetail.members" :key="member.userId" class="mr-2 mb-2">
              {{ member.realName }}
            </el-tag>
          </div>
...
const isManager = computed(() => userStore.userInfo?.roleType === 3)
const currentUserId = computed(() => userStore.userInfo?.userId)

const isAdminFlag = computed(() => isAdmin.value)
const isManagerFlag = computed(() => isManager.value)

const canEditRow = (row: TeamDetail) => {
  if (isAdmin.value) return true
  if (!isManager.value) return false
  return !!currentUserId.value && row.managerId === currentUserId.value
}
...
const resetForm = () => {
  ...
  form.performanceTarget = undefined
  form.memberIds = []
  if (formRef.value) {
...
const handleEdit = async (row: TeamDetail) => {
  ...
  try {
    ...
    if (anyRes.data?.status || anyRes.status) {
      ...
      form.performanceTarget = data.performanceTarget
